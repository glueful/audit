...
namespace Glueful\Extensions\Audit\Tests\Unit\Schema;

use Glueful\Extensions\Audit\Schema\AuditSchemaVerifier;
...
final class SchemaManifestTest extends TestCase
{
    ...
    public function testVerifierClassConformsToTheManifestContract(): void
    {
        ...
        self::assertSame('glueful/audit', (new $class())->source());
    }

    public function testVerifierCoversEveryRecursivelyDiscoveredMigration(): void
    {
        $mapped = (new AuditSchemaVerifier())->migrationBasenames();
    ...
    {
        $g = $this->manifest();
        $candidates = ['glueful/audit' => new \Glueful\Extensions\ExtensionCandidate(
            name: 'glueful/audit',
            provider: $g['provider'],
        ...
        self::assertSame([], $result->errors, 'audit must resolve enabled-alone');
    }
